...
function playSound(name) {
    ...
    try { app.sounds[name].currentTime = 0; app.sounds[name].play(); } catch(e) {}
}
...
window.onload = function() {
    ...
    document.getElementById('mural').onclick = function(e) {
        ...
        if (btn.classList.contains('btn-share')) shareAsImage(id);
        if (btn.classList.contains('btn-vote')) vote(id, btn.getAttribute('data-type'));
    };
    ...
    document.getElementById('post-btn').onclick = function() {
        ...
        client.from('jokes').insert([{ text: txt, author: alias, authorid: app.user.id, color: document.querySelector('.dot.active').getAttribute('data-color'), avatar: app.user.avatar || 'bot1' }]).then(function() {
