...
function createCard(joke) {
    ...
    else el.style.backgroundColor = joke.color || '#fff9c4';
...
async function postJoke() {
    ...
    if (lastPost && (Date.now() - lastPost < 30000)) return showToast('Espera 30s...', 'error');
...
function updateStats() {
    ...
    if (statusDiv) {
        if (isPurgeActive) {
            ...
            statusDiv.innerHTML = '<div style="background:#ff1744; color:#fff; padding:8px; border-radius:5px; text-align:center; margin-bottom:10px; font-weight:bold; font-size:0.9rem;">🚨 PURGA ACTIVA: Quedan ' + hoursLeft + 'h</div>';
        } else {
    ...
    if (pl) {
        pl.innerHTML = worst.map(function(j) { 
            var saveBtn = isPurgeActive 
                ? '<button class="act-btn btn-vote" data-id="' + j.id + '" data-type="save">🛡️ ' + (j.votes_save||0) + '</button>' 
                : '<span style="font-size:0.7rem; color:#aaa;">🔒 ' + (j.votes_save||0) + '</span>';
            return '<li><div style="flex:1;"><span>' + sanitize(j.author) + '</span><div style="font-size:0.75rem; color:#666;">' + sanitize(j.text).substring(0, 40) + '...</div></div><div style="display:flex; flex-direction:column; align-items:end; gap:5px;"><span style="color:#ff1744;">🍅 ' + (j.votes_bad||0) + '</span>' + saveBtn + '</div></li>'; 
        }).join('') || '<li>No hay candidatos a la purga.</li>';
    }
}
